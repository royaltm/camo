...
const updateMethodsRe = require('./update').updateMethodsRe
const UPDATE_METHOD_NOT_ALLOWED_MSG = require('./update').UPDATE_METHOD_NOT_ALLOWED_MSG;
        ...
        return new Promise(function(resolve, reject) {
        	...
        	that.findOne(collection, query).then(function(data) {
        		if (!data) {
                    ...
                    if (update && !options.upsert) {
                        if (!updateMethodsRe.test(update)) {
                            return reject(new Error(UPDATE_METHOD_NOT_ALLOWED_MSG));
                        }
                    ...
                    db.update(query, update, function(error, result) {
                        if (error) return reject(error);
